...
const contentTypeColors: Record<string, string> = {
  blog: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300',
...
const statusLabels: Record<string, { label: string; color: string }> = {
  ...
  approved: { label: 'Disetujui', color: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300' },
  scheduled: { label: 'Dijadwalkan', color: 'bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300' },
  published: { label: 'Dipublikasi', color: 'bg-teal-100 text-teal-800 dark:bg-teal-900/50 dark:text-teal-300' },
}
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence>
            {history.map((item) => (
              ...
              >
                <Card
                  className="border-emerald-200/50 dark:border-emerald-900/50 hover:border-emerald-300 dark:hover:border-emerald-700 transition-colors cursor-pointer h-full"
                    ...
                    {item.tags && (
                      <div className="flex flex-wrap gap-1 mb-2">
                        {item.tags.split(',').slice(0, 3).map((tag, i) => (
                          <span key={i} className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400">
                            #{tag.trim()}
                          </span>
                        ))}
                      </div>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-emerald-700 dark:text-emerald-400">{selectedItem?.title}</DialogTitle>
          </DialogHeader>
          {selectedItem && (
            <div className="space-y-4 mt-4">
              ...
              {editMode ? (
                ...
                <div className="prose prose-sm dark:prose-invert max-w-none prose-headings:text-emerald-700 dark:prose-headings:text-emerald-400 border rounded-lg p-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      ...
                      className={activeVersion === 0 ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : ''}
                    >
                      Versi Terbaru
                    </Button>
                    {selectedItem.versions.map((v, idx) => (
                      <Button
                        ...
                        className={activeVersion === idx + 1 ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : ''}
                      >
