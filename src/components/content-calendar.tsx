...
const platformColors: Record<string, string> = {
  ...
  tiktok: 'bg-rose-100 text-rose-800 dark:bg-orange-900/50 dark:text-orange-300',
...
const statusIcons: Record<string, React.ReactNode> = {
  ...
  sent: <CheckCircle className="size-3 text-rose-500" />,
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Button onClick={() => { setSelectedDay(new Date()); setScheduleDialogOpen(true) }} className="bg-rose-600 hover:bg-rose-700 text-white gap-2">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }}>
        <div className="grid grid-cols-3 gap-3">
          ...
          <Card className="border-rose-200/50 dark:border-orange-900/50">
            <CardContent className="p-3 text-center">
              <div className="text-2xl font-bold text-rose-600">{totalSent}</div>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div
                    key={day}
                    className={`min-h-[80px] sm:min-h-[100px] rounded-lg border p-1.5 cursor-pointer transition-colors hover:border-orange-300 dark:hover:border-orange-700 ${
                      today ? 'border-rose-400 bg-orange-50/50 dark:bg-orange-900/20 dark:border-orange-700' : 'border-muted'
                    }`}
                    onClick={() => { setSelectedDay(new Date(year, month, day)); setScheduleDialogOpen(true) }}
                  >
                    <div className={`text-xs font-medium mb-1 ${today ? 'text-rose-600 dark:text-orange-400' : 'text-muted-foreground'}`}>
                      {day}
                    </div>
      ...
      {schedules.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.3 }}>
          <Card className="border-rose-200/50 dark:border-orange-900/50">
            <CardHeader>
              <CardTitle className="text-sm text-rose-700 dark:text-rose-400">Jadwal Mendatang</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {schedules
                  ...
                  .map(schedule => (
                    <div key={schedule.id} className="flex items-center justify-between gap-3 p-2 rounded-lg border hover:border-orange-300 dark:hover:border-orange-700 transition-colors">
